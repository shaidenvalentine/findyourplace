...
/**
 * The free, share-worthy peak: the breed the user had in mind vs their (still-unnamed)
 * #1 match — overall and by category. The match is never named on the free surface.
 */
export function LifeChangeCompare({
  dreamBreed,
  lifeChange,
  topCount,
}: {
  ...
  const pick = dreamBreed?.trim() ? titleCase(dreamBreed.trim()) : "the breed you had in mind";

  // The #1 match IS the breed they came in wanting — that's a validation moment, not a
  // "jump". Comparing a breed to itself with directional trade-off notes reads as broken math.
        ...
        <CardContent>
          <div className="mb-4 rounded-xl border border-border bg-surface p-5 text-center">
            ...
            <div className="mt-1 text-[11px] uppercase tracking-[0.15em] text-muted-foreground">
              {pick} — your instinct&apos;s fit
            </div>
          </div>
          <p className="text-center text-sm text-muted-foreground">
            {heldTheTop ? (
              <>
                We scored all {topCount} breeds against you, and the top of the list looks{" "}
              ...
              <>
                One breed matched your raw stats slightly harder ({runnerUp}) — but on the things
                you said actually matter, the top of your ranking looks{" "}
                <span className="font-semibold text-foreground">very familiar</span>.
              </>
            )}{" "}
            Unlock to confirm your #1 — and see how the other {Math.max(0, topCount - 1)} stack up.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader>
        <Badge variant="accent" className="w-fit">
          <TrendingUp className="size-3" /> What you&apos;d actually get
        </Badge>
        <CardTitle className="text-lg">{lifeChange.headline}</CardTitle>
      </CardHeader>
      <CardContent>
        {/* Overall dream breed → best */}
        <div className="mb-5 flex items-center justify-between gap-3 rounded-xl border border-border bg-surface p-4">
          <div className="flex-1 text-center">
            ...
            <div className="mt-0.5 text-[11px] text-muted-foreground">{pick}</div>
          </div>
          ...
          <div className="flex-1 text-center">
            ...
            <div className="mt-0.5 text-[11px] text-muted-foreground">your #1 match</div>
          </div>
        </div>
          ...
          {lifeChange.categories.map((c) => (
            <div key={c.label}>
              ...
              <div className="relative h-2 w-full overflow-hidden rounded-full bg-muted">
                ...
                />
                {/* dream-breed tick, drawn on top so the gap to "best" is always visible
                    (and reads correctly even when the user's pick is the stronger one) */}
                <div
